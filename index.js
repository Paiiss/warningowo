  ...
  if (interaction.commandName === "alert") {
    if (interaction.options.get("type").value) {
      if (active_user.includes(interaction.user.id))
        return interaction.reply("You are already active");
      ...
      if (!active_user.includes(interaction.user.id))
        return interaction.reply("You are already inactive");
...
client.on("messageCreate", async (message) => {
  ...
  if (
    ...
    active_user.includes(message.mentions.users.first().id)
  ) {
    ...
    return user.send("Hello, It looks like you were detected as a bot by OwO.");
  }
});
